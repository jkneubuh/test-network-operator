...
export ORG0_CA_CERT=$(kubectl -n ${namespace} get cm/org0-ca-connection-profile -o json | jq -r .binaryData.\"profile.json\" | base64 -d | jq -r .tls.cert)
export ORG1_CA_CERT=$(kubectl -n ${namespace} get cm/org1-ca-connection-profile -o json | jq -r .binaryData.\"profile.json\" | base64 -d | jq -r .tls.cert)
export ORG2_CA_CERT=$(kubectl -n ${namespace} get cm/org2-ca-connection-profile -o json | jq -r .binaryData.\"profile.json\" | base64 -d | jq -r .tls.cert)

# Apply the peer and orderer CRDs, substituting the CA certs.
kubectl kustomize kustomization/network | envsubst | kubectl -n ${namespace} apply -f -
